...
from utils.transliteration_utils import (
...
def SQLite_find_name(name):
    ...
    def execute_query(word):
        session = Session()
        try:
            # Combine the two queries into one using JOIN
            # We select specific columns instead of SELECT *
            # We use meaningful table aliases l2 and l1
            query_builder = text("""
                SELECT 
                    l2.key,
                    l2.type,
                ...
                FROM lgtab2 l2
                JOIN lgtab1 l1 
                    ON l1.stem = l2.root_form 
                    AND l1.model = l2.type
                WHERE l2.key = :word
            """)
            results = session.execute(query_builder, {'word': word}).fetchall()
        except Exception as error:
    ...
    # Try first query with original name
    results = execute_query(name)
    
    # If no results and name ends with 'M', try with 'm'
    if not results and name[-1] == 'M':
        name = name[:-1] + 'm'
        results = execute_query(name)

    # Process results
    for key, type, root_form, refs, data in results:
        if not root_form:  # If root_form is None or empty
            return

        # Extract word reference using regex
        word_refs = re.findall(r",([a-zA-Z]+)", refs)[0]
        
        # Split inflection data
        inflection_words = data.split(':')

        # Transliterate results to IAST
        inflection_wordsIAST = [transliterateSLP1IAST(word) for word in inflection_words]
        query_transliterateIAST = transliterateSLP1IAST(name)

        # Build inflection table
        indices = [i for i, x in enumerate(inflection_wordsIAST) if x == query_transliterateIAST]
        rowtitles = ["Nom", "Acc", "Inst", "Dat", "Abl", "Gen", "Loc", "Voc"]
        coltitles = ["Sg", "Du", "Pl"]
        ...
        outcome.append([
            word_refs, type, row_col_names, 
            inflection_wordsIAST, transliterateSLP1IAST(name)
        ])
...
def test_performance(function, test_words, iterations=100):
    """
    Test the performance of a function over multiple iterations.
    
    Args:
        function: The function to test
        test_words: List of words to test with
...
# Test both versions
test_words = ['gacCAmi', 'devena', 'pratiprasave', 'pramARAni']  # Add your test words here

# Keep the old function for comparison
old_find_name = SQLite_find_nameOLD  # Save the original function
results_old = test_performance(old_find_name, test_words)

# Test new version
results_new = test_performance(SQLite_find_name, test_words)

# Print results
print("Original version:")
print(f"Average time per query: {results_old['mean']:.2f}ms (±{results_old['std_dev']:.2f}ms)")
print(f"Range: {results_old['min']:.2f}ms to {results_old['max']:.2f}ms")

print("\nOptimized version:")
print(f"Average time per query: {results_new['mean']:.2f}ms (±{results_new['std_dev']:.2f}ms)")
print(f"Range: {results_new['min']:.2f}ms to {results_new['max']:.2f}ms")

improvement = ((results_old['mean'] - results_new['mean']) / results_old['mean']) * 100
