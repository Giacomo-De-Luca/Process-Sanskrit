...
| the `-n` lemma rule (`cleanResults:95`) | the whole word | `viṣayin` decays to `viṣayī` |
...
**`rootAnyWord._stamp_whole_word(matches, span, word)`** stamps the whole word on
exactly the entries that resolved `span` itself. Prefix entries keep their own
surface, so they no longer share one with the root; entries carried up from a
*deeper* prefix decomposition carry their own surface and therefore fail the
`span` test, which is what keeps `a` + `vi` + `rati` apart. The root entries still
carry the whole word, so the `-n` rule and the dictionary check see what they
always saw.
...
some analysis already yields that stem, or `pratyakṣam` is reported twice — once as
itself, once as the re-inserted headword.
...
per collateral failure above, so any of the three tempting rewrites fails loudly.
